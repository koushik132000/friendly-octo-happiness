...
#
# Note: No z-unwrapping needed — Zwall confinement prevents chains
#       from spanning the boundary.
#
...
#    (no unwrapping needed — Zwall system)
...
for run in range(NRUNS):
    ...
    for fn in range(1, FN_MAX + 1):
        ...
        for c in range(nc):
            sz        = zs[c * bc : (c + 1) * bc].sum()   # vectorized
            z_com     = sz / bc
...
print(f"Used {NRUNS} independent runs, {FN_MAX - EQUIL_START} equilibrated snapshots each")

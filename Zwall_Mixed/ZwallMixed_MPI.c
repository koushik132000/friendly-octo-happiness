...
/*******************************************************************************
Folded chains are equilibrated by performing MC moves with an excluded volume constraint, bead-bead and bead-wall interactions. The chains interact with a wall only in the z direction. Moves are accepted or rejected based on metropolis algorithm. Generates 1000 configuration files for a system. 
*******************************************************************************/

//Creation Date:27Nov2024
//Last Modified:18May2026
...
14-05-2026: Implementing PBC in xy and z wall constraint
15-05-2026: Corrected sflg to pointer variable
17-05-2026: Bead-Wall interaction is attractive or repulsive depending on the chain number
18-05-2026: Implemented MPI
*/
...
*******************************************************************************/
double Eaa=-0.336;//Bead-Wall attraction
double Ear=0.336;//Bead-Wall repulsion
...
/*******************************************************************************
A site in the bulk has 6 neighbors and near the wall has 5 neighbors
*******************************************************************************/
...
double deltaE(int olds,int news,double Ea)
{
	/***********************************************************************
	...
	***********************************************************************/
	...
	int osflg,nsflg;//Old and new site flags for determining surface proximity
	...
	//Bead-Bead and Bead-Wall interaction energy contributions
...
void fmeval(int cnum,int dindex,int fcalc,double Ea)
{
	fmoves(cnum,dindex);
	scalc=fpos.ex+lx*fpos.ey+lx*ly*fpos.ez;
	dEf=deltaE(fcalc,scalc,Ea);
}	

//Evaluates the terminal move for the last bead in a specific direction
void lmeval(int cnum,int dindex,int lcalc,double Ea)
{
	nmoves(cnum,dindex);
	scalc=npos.ex+lx*npos.ey+lx*ly*npos.ez;
	dEl=deltaE(lcalc,scalc,Ea);
}
...
int main(int argc, char **argv)
{
	...
	//Setting for mixed interaction between chain and a wall
	int ncr=(int)(0.5*nc);//Number of repulsive chains
	double Ea;//Bead-Wall interaction for a chain
	...
	***********************************************************************/
	...
	do{	
		...
		//Chains [0,ncr) are repulsive and [ncr,nc) are attractive
		if (cnum<ncr)
			Ea=Ear;
		else
			Ea=Eaa;

...
#include <mpi.h>
/*******************************************************************************
Folded chains are equilibrated by performing MC moves. The beads interact via a bead-bead attraction energy and an excluded volume constraint. The beads interact with wall with bead-wall interaction energy. Moves are accepted or rejected based on metropolis algorithm. Generates 1000 configuration files for a system. 
*******************************************************************************/

//Creation Date:27Nov2024
//Last Modified:10May2026
...
10-05-2026: Introduces a new function to calculate number of occupied sites around a lattice site.
10-05-2026: Modifies site flag associated with a bead and calculates bead wall interaction energy.
13-05-2026: Implemented MPI
15-05-2026: Corrected global sflg bug in deltaE by changing nneigh to accept sflg as a pointer
15-05-2026: Declared separate local flags in deltaE for the old and new sites.
*/
...
/*******************************************************************************
There are 4 functions other than the main function
...
//Site flag for an occupied site
int sflg;
...
*******************************************************************************/
double Ea=+0.336;//Bead-Wall interaction energy
...
/*******************************************************************************
A site has a maximum of 6, 5, 4 or 3 neighbors depending on its location
1) Internal sites have 6 neighbors
2) Wall sites have 5 eighbors
3) Edge sites have 4 neighbors
4) Corner sites have 3 neighbors
5) Each site has a unique x, y and z layer number associated with it
Each occupied site will have at least 1 neighbor if it is a terminal bead and 2 neighbors if it is an internal bead
*******************************************************************************/
...
int nneigh(int sindex, int *sflg)
{
	...
	*sflg=0;//Initialize all sites as internals
	if (xl==0||xl==lx-1||yl==0||yl==ly-1||zl==0||zl==lz-1)
	{
		//Reset site flag for a surface
		*sflg=1;

		//Resets site flag for 12 edges	
		//Box left edges (4)
		if ((xl==0&&yl==0)||(xl==0&&yl==ly-1)||(xl==0&&zl==0)||(xl==0&&zl==lz-1))
			*sflg=2;
		//Box right edges (4)
		if ((xl==lx-1&&yl==0)||(xl==lx-1&&yl==ly-1)||(xl==lx-1&&zl==0)||(xl==lx-1&&zl==lz-1))
			*sflg=2;
		//Box front-bottom-top and back-bottom-top edges (4)
		if ((yl==0&&zl==0)||(yl==0&&zl==lz-1)||(yl==ly-1&&zl==0)||(yl==ly-1&&zl==lz-1))
			*sflg=2;
		
		//Reset site flag for 8 corners
		//Left side corners (4)
		if ((xl==0&&yl==0&&zl==0)||(xl==0&&yl==0&&zl==lz-1)||(xl==0&&yl==ly-1&&zl==0)||(xl==0&&yl==ly-1&&zl==lz-1))
			*sflg=3;

		//Right side corners (4)
		if ((xl==lx-1&&yl==0&&zl==0)||(xl==lx-1&&yl==0&&zl==lz-1)||(xl==lx-1&&yl==ly-1&&zl==0)||(xl==lx-1&&yl==ly-1&&zl==lz-1))
			*sflg=3;
	}
	/***********************************************************************
	...
	***********************************************************************/
	...
	if (*sflg==0 && nns!=6)
        ...
        exit(EXIT_FAILURE);}
	if (*sflg==1 && nns!=5)
		{printf("Error in surface neighbor calculation: %d\n",nns);
        exit(EXIT_FAILURE);}
	if (*sflg==2 && nns!=4)
		{printf("Error in edge neighbor calculation: %d\n",nns);
        exit(EXIT_FAILURE);}
	if (*sflg==3 && nns!=3)
		{printf("Error in corner neighbor calculation: %d\n",nns);
        exit(EXIT_FAILURE);}
	if(ons>nns)
...
double deltaE(int olds,int news)
{
	...
	***********************************************************************/
	...
	//Bead-Bead and Bead-Wall interaction energy contribution
	osv=nneigh(olds,&osflg);
	Eold+=osv*Eb;//Bead-Bead
	if (osflg > 0)//Bead-Wall
		Eold+=osflg*Ea;
	nsv=nneigh(news,&nsflg);
	Enew+=nsv*Eb;//Bead-Bead
	if (nsflg > 0)//Bead-Wall
		Enew+=nsflg*Ea;
